...
import { deepAssign, deepClone, APP_TITLE, buildSmartRegExp, getAcceleratedUrl, createTextMatcher } from '@/utils'
      ...
      for (const proxy of outbound.outbounds) {
        if (proxy.type === 'Built-in') {
          ...
          if (proxy.type === 'Subscription') {
            ...
            SubscriptionCache[subId]!.forEach((v) => {
              if (isTagMatching(v.tag)) {
                proxiesMap.set(v.tag, cleanOutbound({ ...v }))
              }
            })
          } else {
            ...
            if (_proxy && isTagMatching(_proxy.tag)) {
              ...
              proxiesMap.set(_proxy.tag, cleanOutbound({ ..._proxy }))
            }
          }
        }
      }
